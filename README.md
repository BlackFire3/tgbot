...
- 🧮 **Калькулятор выражений** — `@имя_бота 2500*12 usd rub`, `(100+50)/3 eur`
- 🔔 **Алерты по курсу** — `BTC > $100 000`, `USD < 95 ₽` и т.п., до 10 на пользователя
...
В любом чате введи `@имя_бота <сумма или выражение> <валюта> [целевая]`:
...
@имя_бота 50 €
@имя_бота 2500*12 usd rub
@имя_бота (100+50)/3 eur
```
...
В поле суммы — и в inline-режиме, и при ручном вводе после выбора
направления — поддерживаются арифметические выражения: `+ - * / % ( )`.
Парсер работает на белом списке AST-узлов, без `eval()`.
...
| `/alerts` | Всем | Настроить уведомления о курсах (до 10 алертов) |
...
- `rates.db` — SQLite с тремя таблицами: `rates` (курсы ЦБ), `users` (для рассылки), `alerts` (пороговые уведомления).
- История курсов хранится **30 дней** — старые записи удаляются автоматически.
- Алерты — одноразовые: после срабатывания удаляются автоматически.
- Проверка алертов: BTC — каждые 10 минут (CoinGecko), USD/EUR/KZT — каждый час (ЦБ РФ).
